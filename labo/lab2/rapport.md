...
**min_max_top** correspond au prémier schéma de l'analyse. Cette architecture est composé de blocs bin_lin et d'un bloc marche_normale.

**bin_lin_4to16** Ce bloc est du type flot_don et fait la conversion d'un nombre binaire à un nombre linaire.

**marche_normale** Cette entity correspond au deuxième schéma de l'analyse. Il decrit le comportement de la marche normale.
...
On voit que la vue RTL est pratiquement la même que dans la conception. Le MUX par contre à été séparé en 2.

#### Vue RTL du bin_lin_4to16

<img src="img/rtl_bin_lin.png" style="zoom:50%;" />

Voici un apperçu de la RTL du convertisseur binaire->linaire. C'est une extension du convertisseur du labo prècedent.

#### Vue RTL de la marche normale

<img src="img/rtl_marche_normale.png" style="zoom:80%;" />

Cette RTL est la même que dans la conception à la diffèrence près, que les bus ont été découpés en signal à 1 bit. 
...
Avec la commande 10, toutes les leds sont éteint.

##### Mode allumé fort

![](img/alumee.png)

Avec la commande 11, toutes les leds sont allumée.
...
Ceci démontre la marche normale avec les même valeurs que l'exemple sur la donnée.
On voit que les leds 15à13 et les leds 1à0 sont éteint. Les leds 8à2 (val à min) sont allumée tous les cycles de osc. Les leds sont 12à9 sont allumée un cycle sur deux du signal osc.

##### Marche normale hors interval

![](img/marchenormalehorsinterval.png)

Dès que la valeur est hors de l'interval min à max, toutes les leds sont éteint.
